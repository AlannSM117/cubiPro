...
import { correctTimezoneDate, formatDateTime } from '@/lib/formatters';
  ...
  return (
    <>
      <Header
        ...
        subtitle="Resumen en tiempo real de materia prima y producto terminado"
      />
      
      {/* 4 Tarjetas Superiores */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          label="Volumen ingresado hoy"
          value={stats.volumenIngresadoHoy.toFixed(2)}
          icon={Truck}
          variant="blue"
          suffix="m³"
        />
        <StatCard
          ...
          value={stats.volumenProducidoHoy.toFixed(2)}
          icon={Factory}
          variant="green"
          suffix="m³"
        />
        <StatCard
          label="Total de trozas"
          value={stats.totalTrozas}
          icon={Layers}
          variant="orange"
        />
        <StatCard
          label="Rendimiento general"
          value={`${stats.rendimientoGeneral.toFixed(1)}%`}
          icon={TrendingUp}
          variant="purple"
        />
      </div>
            ...
            {chartData && chartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <XAxis
                      dataKey="fecha"
                      // tickFormatter del XAxis
                      tickFormatter={(val) => {
                        if (!val) return '';
                          try {
                            return correctTimezoneDate(val).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' });
                          } catch { return val; }
                        }
                      }
                      stroke="#9ca3af"
                    ...
                    <Tooltip
                      ...
                      // labelFormatter del Tooltip
                      labelFormatter={(label) => {
                        try {
                            return correctTimezoneDate(label).toLocaleDateString('es-ES', {
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#c1cac7]">
                  ...
                  <th className="font-lexend font-medium text-center text-[14px] text-[#839590] pb-4">Volumen (m³)</th>
                </tr>
              </thead>
              <tbody>
                {entradasRecientes.slice(0, 4).map((entrada) => (
                      <tr key={entrada.id} className="border-b border-[#e0e4e3] last:border-0 hover:bg-gray-400/10 transition-colors">
                        ...
                        <td className="font-lexend font-normal py-4 text-[13px] text-gray-600 text-center pr-2">
